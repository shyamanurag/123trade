...
class OrderManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.zerodha_client:
            logger.info("Γ£à OrderManager initialized with Zerodha client")
        else:
            logger.warning("ΓÜá∩╕Å OrderManager initialized without Zerodha client")
        ...
        if config.get('redis') is not None:
            ...
            self.redis = None
            logger.warning("ΓÜá∩╕Å OrderManager using in-memory fallback (no Redis)")
        ...
        if not self._async_components_initialized:
            try:
                ...
                if hasattr(self.risk_manager, 'async_initialize_event_handlers'):
                    ...
                    logger.info("Γ£à OrderManager: RiskManager event handlers initialized")
                else:
                    logger.warning("ΓÜá∩╕Å OrderManager: RiskManager doesn't have async_initialize_event_handlers")
                
                self._async_components_initialized = True
                logger.info("Γ£à OrderManager: All async components initialized")
            except Exception as e:
                logger.error(f"Γ¥î OrderManager: Failed to initialize async components: {e}")
        ...
        try:
            ...
            try:
                ...
                if symbol in live_market_data:
                    ...
                    if price > 0:
                        logger.debug(f"≡ƒôè Got price for {symbol}: Γé╣{price}")
                        return float(price)
                    else:
                        logger.warning(f"ΓÜá∩╕Å Zero price for {symbol}")
                        return None
                else:
                    logger.warning(f"ΓÜá∩╕Å Symbol {symbol} not found in TrueData cache")
                    return None
                    
            except ImportError:
                logger.error("Γ¥î TrueData client not available")
                return None
    ...
    async def _execute_market_order(self, order: Order) -> Dict[str, Any]:
        ...
        try:
            ...
            if not current_price:
                logger.warning(f"ΓÜá∩╕Å No current price available for {order.symbol}")
                return {
                    'status': 'REJECTED',
            ...
            try:
                ...
                if not zerodha_client:
                    ...
                    logger.error("Γ¥î CRITICAL: No Zerodha client available in OrderManager")
                    logger.error("Γ¥î SAFETY: No fallback client access - real broker required")
                    return {
                        'status': 'REJECTED',
                ...
                if not zerodha_client:
                    logger.error("Γ¥î No Zerodha client available")
                    return {
                        'status': 'REJECTED',
                        'reason': 'NO_BROKER_CLIENT',
                        'order_id': order.order_id,
                ...
                logger.info(f"≡ƒÜÇ Placing market order via Zerodha: {order.symbol} {order.quantity} @ MARKET")
                
                # Place order through Zerodha
                broker_order_id = await zerodha_client.place_order(order_params)
                
                if broker_order_id:
                    logger.info(f"Γ£à Market order placed successfully: {broker_order_id}")
                    return {
                        'status': 'FILLED',
                    ...
                    logger.error(f"Γ¥î Failed to place market order for {order.symbol}")
                    return {
                        'status': 'REJECTED',
                ...
                logger.error(f"Γ¥î Error executing market order: {e}")
                return {
                    'status': 'REJECTED',
            ...
            try:
                ...
                if not zerodha_client:
                    ...
                    if hasattr(orchestrator, 'zerodha_client') and orchestrator.zerodha_client:
                        ...
                        logger.info("≡ƒöä Using Zerodha client from orchestrator fallback")
                    else:
                        logger.error("Γ¥î No Zerodha client available in OrderManager or orchestrator")
                        return {
                            'status': 'REJECTED',
                            'reason': 'NO_BROKER_CLIENT',
                            'order_id': order.order_id,
                ...
                logger.info(f"≡ƒÜÇ Placing limit order via Zerodha: {order.symbol} {order.quantity} @ Γé╣{order.price}")
                
                # Place order through Zerodha
                broker_order_id = await zerodha_client.place_order(order_params)
                
                if broker_order_id:
                    logger.info(f"Γ£à Limit order placed successfully: {broker_order_id}")
                    return {
                        'status': 'OPEN',
                        'broker_order_id': broker_order_id,
                        'order_id': order.order_id,
                    ...
                    logger.error(f"Γ¥î Failed to place limit order for {order.symbol}")
                    return {
                        'status': 'REJECTED',
                        'reason': 'BROKER_REJECTION',
                        'order_id': order.order_id,
                ...
                logger.error(f"Γ¥î Error executing limit order: {e}")
                return {
                    'status': 'REJECTED',
                    'reason': 'EXECUTION_ERROR',
                    'order_id': order.order_id,
    ...
    async def _ensure_background_tasks_running(self):
        ...
        try:
            ...
            if self.redis:
                ...
                logger.info(f"Γ£à Trade stored in Redis for analytics: {trade_key}")
            else:
                logger.warning("ΓÜá∩╕Å Redis not available, trade not stored for analytics")
